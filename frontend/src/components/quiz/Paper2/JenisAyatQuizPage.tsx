...
export default function JenisAyatQuizPage() {
  const { t } = useLanguage();
  const { user } = useAuth();
  ...
  const fetchQuestions = async (forceNew = false, retry = false, retrySessionId?: string) => {
    ...
    try {
      if (retry && retrySessionId) {
        const data = await retryComprehension(retrySessionId, "jenis_ayat", userId) as StartResponse;
      ...
      if (!forceNew) {
        const res = await fetch(
          `${API_URL}/comprehension/resume-latest?quiz_type=jenis_ayat&user_id=${userId}`
        );
      ...
      const data = (await startComprehension("jenis_ayat", userId)) as StartResponse;
      setSessionId(data.session_id);
      setQuestions(data.questions);
  ...
  return (
    <Layout>
      <div className="container no-top-gap">
        ...
        {/* Header */}
        <div className="hero">
          <h1>Jenis Ayat Quiz</h1>
          <p>Menguji pemahaman jenis dan struktur ayat.</p>
        </div>
            ...
            {questions.map((q, index) => {
              ...
              return (
                ...
                >
                  ...
                  <p style={{ whiteSpace: "pre-line" }}>{q.question_text}</p>

                  {/* Dropdown for identify-type questions, textarea for transform questions */}
                  {hasOptions ? (
