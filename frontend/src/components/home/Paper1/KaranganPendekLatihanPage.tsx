...
export default function KaranganPendekLatihanPage() {
  ...
  const [task, setTask] = useState<ShortEssayTask | null>(null);
  ...
  async function loadGeneratedQuestion() {
    ...
    try {
      ...
      setTask({
        instruction: t("shortGeneratedInstruction"),
        stimulus: [t("generatedPracticeNotice")],
        question: generatedQuestion,
        placeholder: t("shortGeneratedPlaceholder"),
      });
  ...
  async function handleSubmit() {
    ...
    if (answer.trim().split(/\s+/).length < 20) {
      setError(t("shortEssayTooShortError"));
      return;
    }
  ...
  return (
    <Layout>
      ...
      <div className="page-header">
        <h1>
          {t("shortEssayPracticeTitle")} - {t(difficulty)}
        </h1>
      ...
      <div className="chapter-card card-base" style={{ marginBottom: "25px" }}>
        {isGenerating ? (
          ...
          <>
            {task?.question && (
              <div style={{ lineHeight: "1.8", textAlign: "justify", marginTop: "12px" }}>
                {(() => {
      ...
      <div className="text-area-karangan">
        <textarea
          placeholder={task?.placeholder || t("shortGeneratedPlaceholder")}
          value={answer}
      ...
      {result && (
        <div className="chapter-card card-base" style={{ marginTop: "25px" }}>
