...
function extractBinaAyatWord(questionText: string): string {
  const text = questionText.trim();

  const quotedMatch = text.match(/["“”']([^"“”']+)["“”']/);
  if (quotedMatch) {
    return quotedMatch[1].replace(/[.,!?;:]$/, "").trim();
  }

  const kataMatch = text.match(/(?:kata|perkataan)\s*(?:berikut)?\s*:?\s*([A-Za-zÀ-ž-]+)/i);
  if (kataMatch) {
    return kataMatch[1].replace(/[.,!?;:]$/, "").trim();
  }

  const colonMatch = text.match(/:\s*([^.!?\n]+)/);
  if (colonMatch) {
    return colonMatch[1].replace(/[.,!?;:]$/, "").trim();
  }

  return text;
}

export default function BinaAyatLatihanPage() {
          ...
          questions.map((question, index) => {
            ...
            return (
              ...
              >
                ...
                <h4>
                  Kata:{" "}
                  <strong className="latihan-highlight-word">
                    {extractBinaAyatWord(question.question_text)}.
                  </strong>
                </h4>
