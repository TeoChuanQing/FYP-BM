...
function renderLatihanHighlightedQuestion(questionText: string) {
  ...
  return parts.map((part, index) => {
    ...
    const isAsterisked = part.startsWith("*") && part.endsWith("*");

    if (isQuoted || isAsterisked) {
      const cleanWord = part
        .replace(/^["“*]/, "")
        .replace(/["”*]$/, "");

      return (
        <strong key={index} className="latihan-highlight-word">
          {cleanWord}
        </strong>
      );
    }
...
export default function GolonganKataLatihanPage() {
            ...
            return (
              ...
              >
                ...
                <p className="practice-question-text">
                  {renderLatihanHighlightedQuestion(question.question_text)}
                </p>
